/* ISO 4217, 货币代码表, From Wikipedia
*/
#include "string.h"
#include "VposFace.h"

static struct {
    char   szAlphaCode[4];      // 字母代码
    short  iDigitCode;          // 数字代码
    uchar  ucDecimalPosition;   // 小数点位置(0-3)
} sg_aCurrencyCodeTable[] = {
    ...
    {"ANG", 532, 2}, //Netherlands Antillean guilder  Cura?ao,  Sint Maarten 
    ...
    {"STD", 678, 0}, //S?o Tomé and Príncipe dobra  S?o Tomé and Príncipe 
    ...
    {"VND", 704, 0}, //Vietnamese ??ng  Vietnam 
...
{
	int i;
	for(i=0; i<sizeof(sg_aCurrencyCodeTable)/sizeof(sg_aCurrencyCodeTable[0]); i++) {
		if(sg_aCurrencyCodeTable[i].iDigitCode == iDigitCode) {
			strcpy((char *)pszAlphaCode, sg_aCurrencyCodeTable[i].szAlphaCode);
			*piDecimalPosition = sg_aCurrencyCodeTable[i].ucDecimalPosition;
			return(0);
		}
	}
	// 没找到, 填缺省值, ISO4217在未知时用"XXX"标记, 但eCash要用"   "标记
	ASSERT(0);
//	strcpy(pszAlphaCode, "XXX");
//	*piDecimalPosition = 0;
	strcpy(pszAlphaCode, "   ");
	*piDecimalPosition = 2;
	return(1);
