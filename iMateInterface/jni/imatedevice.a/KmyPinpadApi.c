...
int KmyPinpad_Reset(int initFlag)
{
	...
	if (initFlag)
		len = 2;
	return kmyPinpadComm(sendBytes, len, NULL, NULL, 2);
}
...
int KmyPinpad_InputPinblock(int is3des, int isAutoReturn, int masterIndex, int workingIndex, char* cardNo, int pinLength, uchar *pinblock, int timeout)
{
	...
	while (time(NULL) < tm) {
		...
		int receivedLength;
		ret = syncCommon(sendBytes, 3, receivedBytes, &receivedLength, 1);
		if (ret)
			return ret;
...
static int kmyPinpadComm(uchar *in, int inLength, uchar *out, int *outLength, int timeout)
{
	...
	int ret = syncCommon(sendBytes, 5+packLength, NULL, NULL, 1);
	if (ret)
		return -1;
	...
	while (time(NULL) < m) {
		...
		uchar tmpBytes[600];
        int theLen;
		int ret = syncCommon(sendBytes, 3, tmpBytes, &theLen, 1);
		if (ret)
			return -2;
