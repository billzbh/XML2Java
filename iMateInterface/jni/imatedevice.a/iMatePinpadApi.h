...
void Pinpad_SetPinpadModel(JNIEnv *env, int pinpadModel);

// 设置算法类型，用于下载masterkey或workingkey之前调用，仅用于信雅达的Pinpad
// in  : mode		: 主密钥类型包括，DECRYPT_KEY_MODE，ENCRYPT_KEY_MODE，PIN_KEY_MODE
void Pinpad_SetKeyMode(JNIEnv *env, int mode);

// Pinpad上电
int Pinpad_PowerOn(JNIEnv *env);

// Pinpad下电
int Pinpad_PowerOff(JNIEnv *env);
...
// ret : 0     		: 成功
//       1     		: 失败
void Pinpad_ParamSetup(JNIEnv *env, char *paramName, unsigned char* value);

// 取消密码输入操作，用于取消密码输入
void Pinpad_Cancel(JNIEnv *env);

// Pinpad复位自检
// in  : initFlag  	: 1-清除Pinpad中的密钥, 0-不清除密钥
// ret : 0			: 成功
//		 其它		: 失败
int Pinpad_Reset(JNIEnv *env, int initFlag);
...
// ret : 0					: 成功
//		 其它				: 失败
int Pinpad_GetVersion(JNIEnv *env, unsigned char *firmwareVersion, int *versionLength);
...
// ret : 0			: 成功
//		 其它		: 失败
int Pinpad_DownloadMasterKey(JNIEnv *env, int is3des, int index, unsigned char* masterKey, int keyLength);
...
// ret : 0				: 成功
//		 其它			: 失败
int Pinpad_DownloadWorkingKey(JNIEnv *env, int is3des, int masterIndex, int workingIndex, unsigned char* workingKey, int keyLength);
...
// ret : 0				: 成功
//		 其它			: 失败
int Pinpad_InputPinblock(JNIEnv *env, int is3des, int isAutoReturn, int masterIndex, int workingIndex, char* cardNo, int pinLength, unsigned char *pinblock, int timeout);
...
// ret : 0				: 成功
//		 其它			: 失败
int Pinpad_Encrypt(JNIEnv *env, int is3des, int algo, int masterIndex, int workingIndex, unsigned char*  inData, int dataLength, unsigned char * outData);
...
// ret : 0				: 成功
//		 其它			: 失败
int Pinpad_Mac(JNIEnv *env, int is3des, int masterIndex, int workingIndex, unsigned char* data, int dataLength, unsigned char *mac);
