...
static void pininputCancel(void)
{
	...
	int ret = syncCommonEx(sendBytes, 8, NULL, NULL, 1);
	if (ret)
		return;
	...
	sendBytes[1] = 0x00;
	sendBytes[2] = 4;
	syncCommonEx(sendBytes, 3, NULL, NULL, 1);
}
...
static int xydPinpadComm(uchar *in, int inLength, uchar *out, int *outLength, int timeout)
{
	...
	int ret = syncCommonEx(sendBytes, 5 + inLength + 1, NULL, NULL, 1);
	if (ret)
		return ret;
	...
	while (time(NULL) < m) {
        ...
        int theLen;
		ret = syncCommonEx(sendBytes, 3, tmpBytes, &theLen, 1);
		if (ret)
			return ret;
